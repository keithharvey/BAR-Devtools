...
# TIP carries two regen-volatile commits toward its tip (fmt-mig + merge of
# $BASE) — regenerated by gen-manifest/build, backed out via strip + retrack.
# Layer homes come from the 7 real commits below them; layer content from the
# migrated tip, so each PR diff is noise-free against the migrated base.
# The type-migration stack (#8235) merges first; this stack then retargets to
# master unchanged.
...
# Migrated line the stack builds on: origin/fmt-llm until #8235 lands, then
# upstream/master. Back out of the migrated design entirely by setting this
# to upstream/master (the tip commits regenerate as no-ops-ish contributor flow).
...
cmd_gen_manifest() {
    ensure_migrated
    local real mb; real=$(real_tip); mb=$(git_bar merge-base "$BASE" "$real")
    step "Deriving manifest: homes from ${mb:0:10}..$real, files from $BASE..$TIP"
    declare -A LBL; local i=0 c
    for c in $(git_bar rev-list --reverse "$mb..$real"); do i=$((i+1)); LBL[$c]=$i; done
    : > "$MANIFEST"
    local f home chunks
    while read -r f; do
        [ -z "$f" ] && continue
        ...
        chunks=""
        for c in $(git_bar log --format='%H' "$mb..$real" -- "$f"); do chunks="$chunks ${LBL[$c]}"; done
        home=$(echo $chunks | tr ' ' '\n' | sort -un | head -1)
        if [ -z "$home" ]; then
            err "no layer home for $f — differs from $BASE but untouched by the real commits"
            err "  (fmt-mig/stylua skew vs $BASE_TRACK, or a merge resolution outside the feature)"
            exit 1
        fi
        printf '%s\t%s\n' "$home" "$f" >> "$MANIFEST"
...
# ── fold-stylua: one-time — bake stylua into the real commits' new .lua files ─
# stylua is runtime-neutral and idempotent, so the regen fmt-mig commit stops
# touching new files' formatting; the codemod transforms stay regen-volatile.
cmd_fold_stylua() {
    local real mb; real=$(real_tip); mb=$(git_bar merge-base "$UPSTREAM_REMOTE/master" "$real")
    step "Folding stylua into new-file content of the real commits (backup: ${TIP}_prefold)"
    git_bar branch -f "${TIP}_prefold" "$TIP"
    git_bar checkout --force -B _sharing_fold "$real" >/dev/null 2>&1
    local f files=""
    while read -r f; do
        case "$f" in *.lua) ;; *) continue ;; esac
        git_bar cat-file -e "$mb:$f" 2>/dev/null && continue
        git_bar cat-file -e "$real:$f" 2>/dev/null && files="$files $f"
    done < <(cut -f2 "$MANIFEST")
    [ -n "$files" ] && (cd "$BAR" && lx --lua-version 5.1 exec stylua $files)
    if git_bar diff --quiet; then
        warn "stylua already folded — nothing to do"
        git_bar checkout --force "$TIP" >/dev/null 2>&1
        git_bar branch -D _sharing_fold >/dev/null
        return 0
    fi
    git_bar commit -q -am "stylua fold"
    # rewrite the real commits onto their own base, new files now styled
    git_bar checkout --force -B _sharing_build "$mb" >/dev/null 2>&1
    local l msg
    for l in $(layer_ids); do
        msg=$(layer_message "$l")
        while read -r f; do
            [ -z "$f" ] && continue
            if git_bar cat-file -e "_sharing_fold:$f" 2>/dev/null; then
                git_bar checkout _sharing_fold -- "$f"
            else
                git_bar rm -q --ignore-unmatch "$f" >/dev/null
            fi
        done < <(files_for "$l")
    ...
    git_bar branch -f "$TIP" HEAD
    git_bar checkout --force "$TIP" >/dev/null 2>&1
    git_bar branch -D _sharing_build _sharing_fold >/dev/null
    ok "$TIP real commits rewritten with styled new files ($(git_bar rev-parse --short "$TIP")) — migration commits return on next build"
}
...
# ── regen commits: TIP = real commits + fmt-mig + merge $BASE ────────────────
real_tip() {  # TIP minus the regen-volatile commits
    local n=0
    while git_bar log -1 --format=%s "$TIP~$n" 2>/dev/null | grep -qF "$REGEN_MARK"; do n=$((n+1)); done
    git_bar rev-parse "$TIP~$n"
}
...
preflight() {
    ...
    if git_bar log --format=%s "$(git_bar merge-base "$BASE" "$TIP")..$(real_tip)" | grep -qF "$REGEN_MARK"; then
        err "regen commits buried under real commits on $TIP — rebase your work below them"; exit 1
    fi
    return 0
}

# gen-manifest/build regenerate the migration commits fresh; check/verify/
# describe/push/update-prs run against the tip as last built.
ensure_migrated() {
    [ -n "$_MIGRATED" ] && return 0
    step "Regenerating migration commits on $TIP (fmt-mig + merge $BASE_TRACK) — the contributor flow"
    git_bar checkout --force "$TIP" >/dev/null 2>&1
    git_bar reset --hard -q "$(real_tip)"
...
cmd_rebase() {
    ...
    git_bar checkout --force "$TIP" >/dev/null 2>&1
    git_bar reset --hard -q "$(real_tip)"   # only the real commits rebase
...
usage() { err "usage: generate.sh <rebase|gen-manifest|check|build|fold-stylua|verify|describe [layer]|pr-body <layer>|push|update-prs> ..."; exit 1; }
[ $# -eq 0 ] && usage
